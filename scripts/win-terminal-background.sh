...
blur_img() {
    ...
    blur="0x$blur_sigma"
    output="$temp_dir/$blur-$(basename "$bg_image_wsl_path")"
    magick "$bg_image_wsl_path" -blur "$blur" "$output"
    output_win_path=$(wslpath -w "$output" | tr "\\" "/" 2> /dev/null)
    new_json=$(echo "$json_file" | jq ".profiles.defaults.backgroundImage = \"$output_win_path\"")
...
if [[ $1 = "--unblur" ]]; then
    bg_image_win_path=$(echo "$json_file" | jq -r '.profiles.defaults.backgroundImageOriginal')
    new_json=$(echo "$json_file" | jq ".profiles.defaults.backgroundImage = \"$bg_image_win_path\" | .profiles.defaults.backgroundImageOriginal = \"$bg_image_win_path\"")
    echo "$new_json" > "$win_terminal_settings"
    exit 0
fi
...
if [[ $1 = "--rotate" ]]; then
    ...
    new_json=$(echo "$json_file" | jq ".profiles.defaults.backgroundImage = \"$new_random_image\" | .profiles.defaults.backgroundImageOriginal = \"$new_random_image\"")
    echo "$new_json" > "$win_terminal_settings"
    exit 0
fi
