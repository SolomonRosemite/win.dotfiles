...
if [[ $1 = "--rotate" ]]; then
    ...
    new_random_image=$(find $dirs -not -path '*/.git/*' -not -name 'README.md' -type f | shuf -n 1 | xargs wslpath -w | tr "\\" "/" 2> /dev/null)
...
if [[ $1 = "--select" ]]; then
    dirs=$(echo "$json_file" | jq '.profiles.defaults.sources[]' | xargs -I{} wslpath {})
    new_random_image=$(find $dirs -not -path '*/.git/*' -not -name 'README.md' -type f | fzf | xargs -I {} wslpath -w "{}" | tr "\\" "/" 2> /dev/null)
